import"./hoisted.bnrPlIK7.js";const h={threshold:.1,rootMargin:"0px 0px -50px 0px"},c=new IntersectionObserver(e=>{e.forEach(o=>{o.isIntersecting&&(o.target.classList.add("animate"),c.unobserve(o.target))})},h);document.addEventListener("DOMContentLoaded",()=>{document.querySelectorAll("section").forEach(o=>{o.classList.add("animate-on-scroll"),c.observe(o)})});const i=["DevOps Engineer","Platform Engineer","Site Reliability Engineer","Cloud Engineer"];let a=0,s=0,n=!1;function d(){const e=document.getElementById("typing-text"),o=i[a];n?(e.textContent=o.substring(0,s-1),s--,s===0&&(n=!1,a=(a+1)%i.length)):(e.textContent=o.substring(0,s+1),s++,s===o.length&&setTimeout(()=>n=!0,2e3)),setTimeout(d,n?50:100)}document.addEventListener("DOMContentLoaded",()=>{setTimeout(d,1e3)});function f(){document.getElementById("demo-modal").classList.add("hidden"),document.body.style.overflow="auto"}document.getElementById("demo-modal").addEventListener("click",function(e){e.target===this&&f()});function v(){document.getElementById("blog-modal").classList.add("hidden"),document.body.style.overflow="auto"}document.getElementById("blog-modal").addEventListener("click",function(e){e.target===this&&v()});function y(){document.getElementById("testimonial-modal").classList.add("hidden"),document.body.style.overflow="auto"}document.getElementById("testimonial-modal").addEventListener("click",function(e){e.target===this&&y()});function E(){}document.addEventListener("DOMContentLoaded",E);const u="sonbarse17",x=`https://api.github.com/users/${u}`,w=`https://api.github.com/users/${u}/repos?sort=updated&per_page=100`;async function I(){try{const o=await(await fetch(x)).json();document.getElementById("repo-count").textContent=o.public_repos,document.getElementById("follower-count").textContent=o.followers,document.getElementById("following-count").textContent=o.following;const l=await(await fetch(w)).json(),r=["sushant-portfolio","DevOps-Projects","AWS-Infrastructure","Kubernetes-Deployment","CI-CD-Pipeline","Terraform-Modules"],g=l.filter(t=>!t.fork&&r.includes(t.name)).concat(l.filter(t=>!t.fork&&!r.includes(t.name)).sort((t,b)=>b.stargazers_count-t.stargazers_count)).slice(0,6),p=document.getElementById("repos-list");p.innerHTML=g.map(t=>`
        <div class="repo-card">
